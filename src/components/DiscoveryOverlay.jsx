...
const CATEGORY_META = {
  ...
  food:         { label: 'Food',         emoji: '🍽️', color: '#fb923c', bg: 'rgba(251,146,60,0.15)' },
...
// Ordered list matching the spec
const CATEGORY_ORDER = [
  'History', 'Art', 'Subculture', 'Architecture', 'Nature', 'Food',
  'Nightlife', 'Religion', 'Urban', 'Views', 'Family', 'Culture',
];
...
// ── POI image component with fallback ───────────────────────────────
const PoiImage = React.memo(({ src, name, emoji, catInfo }) => {
  ...
  return (
    <div className="w-full aspect-[4/3] overflow-hidden relative bg-slate-800">
...
const DiscoveryOverlay = ({ isOpen, onClose, onNavigateToTour }) => {
  ...
  const filteredPois = useMemo(() => {
    ...
    // Category filter (OR logic: show POI if it matches any active category)
  ...
  return (
    <div className="fixed inset-0 z-[600] bg-slate-950/90 backdrop-blur-md flex flex-col pointer-events-auto">
      {/* ── Header ─────────────────────────────────────────────── */}
              ...
              return (
                ...
                >
                  ...
                  <div className="relative">
                    ...
                    {imgSrc && poi.emoji && (
                      <div className="absolute top-2 left-2 w-8 h-8 rounded-xl bg-black/50 backdrop-blur-sm flex items-center justify-center text-lg">
                        {poi.emoji}
                      </div>
                    )}
                    ...
                    {/* Distance badge (shown when sorting by nearby) */}
                    {dist && (
